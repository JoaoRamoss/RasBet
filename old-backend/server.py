...
games = GameDB("sql.db")
backend = Backend(games, "sql.db")
games.set_backend(backend)
...
def api_register_user():
	if request.method == "POST":
		...
		if response == backend.VALID_USER:
			print("Complete")
			session["nif"] = nif
...
def api_register_specialist():
	if request.method == "POST":
		...
		if response == backend.VALID_SPECIALIST:
			print("Complete")
			session["nif"] = nif
...
def api_register_admin():
	if request.method == "POST":
		...
		if response == backend.VALID_ADMIN:
			print("Complete")
			session["nif"] = nif
...
def api_login():
    if request.method == "GET":
        ...
        if response == backend.VALID_USER:
            session["nif"] = nif
            session["logged_in"] = USER
        elif response == backend.VALID_SPECIALIST:
            session["nif"] = nif
            session["logged_in"] = SPECIALIST
        elif response == backend.VALID_ADMIN:
            session["nif"] = nif
            session["logged_in"] = ADMIN
        return jsonify({"response" : response})
    return abort(404)
...
def api_get_active_games(sport):
	if request.method == "GET":
		if is_logged_in() == USER:
			...
			if sport == "football":
				response, data = games.get_games("FOOTBALL", state="ACTIVE")
				return jsonify({"response" : response, "data" : data})
...
def api_get_wait_games(sport):
	if request.method == "GET":
		if is_logged_in() == SPECIALIST or is_logged_in() == ADMIN:
			...
			if sport == "football":
				response, data = games.get_games_and_outcomes("FOOTBALL")
				return jsonify({"response" : response, "data" : data})
			else: return jsonify({"response" : "unknown_sport"})
		else: abort(401)
	return abort(404)
...
def api_get_all_games(sport):
	if request.method == "GET":
		if is_logged_in() == ADMIN:
			print(">> Get all games")
			if sport == "football":
				response, data = games.get_games("FOOTBALL", state="ANY")
				return jsonify({"response" : response, "data" : data})
			else: return jsonify({"response" : "unknown_sport"})
		else: abort(401)
	return abort(404)
...
def api_set_games_odds():
	if request.method == "POST":
		if is_logged_in() == SPECIALIST or is_logged_in() == ADMIN:
			...
			data = request.get_json()
			response = games.set_games_odds("FOOTBALL", data["ids"], data["odds"])
			return jsonify({"response" : response})
		else: abort(401)
	return abort(404)
...
def api_set_games_states():
	if request.method == "POST":
		if is_logged_in() == ADMIN:
			...
			data = request.get_json()
			response = games.set_games_states("FOOTBALL", data["ids"], data["states"])
			return jsonify({"response" : response})
		else: abort(401)
	return abort(404)
...
def run_server():
	...
	if "-rg" in sys.argv: games.reset_games()
	if any(["-apitimeout=" in v for v in sys.argv]): games.api_timeout = int(sys.argv[["-apitimeout=" in v for v in sys.argv].index(True)][12:])
